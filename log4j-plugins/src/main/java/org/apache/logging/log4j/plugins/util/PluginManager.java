...
public class PluginManager {
    ...
    public void collectPlugins(final List<String> packages) {
        ...
        Map<String, List<PluginType<?>>> builtInPlugins = pluginRegistry.loadFromMainClassLoader();
        if (builtInPlugins.isEmpty()) {
            ...
            builtInPlugins = pluginRegistry.loadFromPackage(LOG4J_PACKAGES);
        }
        mergeByName(newPlugins, builtInPlugins.get(categoryLowerCase));

        // Next, iterate any Log4j2Plugin.dat files from OSGi Bundles
        for (final Map<String, List<PluginType<?>>> pluginsByCategory : pluginRegistry.getPluginsByCategoryByBundleId().values()) {
        ...
        for (final String pkg : PACKAGES) {
            mergeByName(newPlugins, pluginRegistry.loadFromPackage(pkg).get(categoryLowerCase));
        }
