...
public class BurstFilterTest {
    ...
    public void test() throws Exception {
        ...
        for (int i = 0; i < 110; i++) {
            ...
            logger.info("Logging 110 messages, should only see 100 logs # {}", box(i + 1));
        ...
        for (int i = 0; i < 110; i++) {
            logger.info("Waited 1.5 seconds and trying to log again, should see more than 0 and less than 100 #{}", box(i + 1));
        }
        ...
        for (int i = 0; i < 110; i++) {
            logger.debug(
                "TEST FAILED! Logging 110 debug messages, shouldn't see any of them because they are debugs #{}", box(i + 1));
        }
        ...
        for (int i = 0; i < 110; i++) {
            logger.warn("Logging 110 warn messages, should see all of them because they are warns #{}", box(i + 1));
        }
        ...
        for (int i = 0; i < 110; i++) {
            logger.error("Logging 110 error messages, should see all of them because they are errors #{}", box(i + 1));
        }
        ...
        for (int i = 0; i < 110; i++) {
            logger.fatal("Logging 110 fatal messages, should see all of them because they are fatals #{}", box(i + 1));
        }
        ...
        for (int i = 0; i < 110; i++) {
            logger.debug("Waited 3+ seconds, should see 100 logs #{}", box(i + 1));
        }
        msgs = app.getMessages();
        assertEquals("Incorrect message count. Should be 100, actual " + msgs.size(), 100, msgs.size());
        app.clear();
